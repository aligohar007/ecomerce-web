<template>
  <article class="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-200">
    <!-- Image -->
    <div class="w-full h-44 sm:h-48 lg:h-70 overflow-hidden">
      <img :src="card.img" :alt="card.title"
        class="w-full h-full object-fit transform hover:scale-105 transition-transform duration-400" />
    </div>
    ...
    <div class="p-4 text-center">
      ...
      <p class="text-xs text-gray-500 mb-5 line-clamp-2">
        {{ card.subtext }}
      </p>
        
       <div>
          <a class="bg-[#fea928] text-white px-2 py-2 rounded-2xl mt-4 mb-3 ">{{ card.btn }}</a>
       </div>
